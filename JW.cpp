...
int solver::DPLL(Formula formula) 
{
  ...
  vector<double> weightedCounter(formula.literalFrequency.size());
  vector<int> addedLiterals;
  int weight = 2;
  for(int i = 0; i < formula.clauses.size(); i++)
  {
    for (int j = 0; j < formula.clauses[i].size(); j++) 
    {
      int clauseSize = formula.clauses[i].size();
      int negateSize = clauseSize * (-1);
      double operation = pow(weight, negateSize);
      if (count(addedLiterals.begin(), addedLiterals.end(), formula.clauses[i][j] / 2) > 0)
      {
        weightedCounter[formula.clauses[i][j] / 2] = weightedCounter[formula.clauses[i][j] / 2] + operation;
      }
      else
      {
        addedLiterals.push_back(formula.clauses[i][j] / 2);
        weightedCounter[formula.clauses[i][j] / 2] = operation;
      }
    }
  }


  int i = distance(weightedCounter.begin(), max_element(weightedCounter.begin(), weightedCounter.end()));
  ...
  for (int j = 0; j < 2; j++) 
  {
    ...
    if (j == 1) 
    {
      newFormula.literals[i] = 0; 
    }
    else                  
    {
      newFormula.literals[i] = 1; 
    }
